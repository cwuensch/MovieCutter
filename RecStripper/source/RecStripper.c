    ...
    if (ShowConfirmationDialog(MessageStr))
    {
      for (CurRecNr = 0; CurRecNr < NrRecFiles; CurRecNr++)
      {
        ...
        else if(FolderEntry.attr == ATTR_NORMAL)
        {
          ...
          TAP_SPrint(CommandLine, sizeof(CommandLine), "( rm /tmp/RecStrip.* ; " RECSTRIPPATH "/RecStrip -s -e \"%s/%s\" \"%s/%s\" 2>&1 > /tmp/RecStrip.log ; echo $? > /tmp/RecStrip.out ) & echo $!", TAPFSROOT RECDIR, CurRecName2, TAPFSROOT OUTDIR, OutRecName2);
