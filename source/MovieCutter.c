...
TAP_Main(void)
{
  ...
  WriteLogMC(PROGRAM_NAME, LogString);
  word FwYear;
  byte FwMonth, FwDay, FwDayOfWeek;
  ...
  // Load Fonts
  if(!(FMUC_LoadFontFile("Calibri_10.ufnt", &Calibri_10_FontDataUC) && FMUC_LoadFontFile("Calibri_12.ufnt", &Calibri_12_FontDataUC) && FMUC_LoadFontFile("Calibri_14.ufnt", &Calibri_14_FontDataUC)))
  {
          ...
          if((param1 == RKEY_Ok) && (OSDMenuMessageBoxLastButton() == 0))
          {
            ...
            switch(ActionMenuItem)
            {
              ...
              case MI_ImportBookmarks:
                (BookmarkMode) ? ExportSegmentsToBookmarks() : AddBookmarksToSegmentList();
                break;
            }
          }
          else
...
//          MinuteJumpBlocks = 0;  // nicht unbedingt nötig
          ...
          if(!PlayInfo.file || !PlayInfo.file->name[0])
          {
            PlaybackName[0] = '\0';
            break;
          }
...
//              PlaybackRepeatSet(TRUE);
          ...
          WriteLogMC(PROGRAM_NAME,
              (HDVideo) ? "Type of recording: HD" : "Type of recording: SD");
...
//          LastTimeStamp = NULL;
          ...
          if(GetRecDateFromInf(PlaybackName, &RecDateTime))
          {
            ...
            if(TimeSinceRec <= UpTime + 1) RebootMessage = TRUE;

#ifdef FULLDEBUG
            if(RecDateTime > 0xd0790000) RecDateTime = TF2UnixTime(RecDateTime);
            TAP_PrintNet("Reboot-Check (%s): TimeSinceRec=%u, UpTime=%u, RecDateTime=%s",
                (RebootMessage) ? "TRUE" : "FALSE", TimeSinceRec, UpTime, asctime(localtime(&RecDateTime)));
#endif
          }
...
//                case MI_DeleteSegment:       MovieCutterDeleteSegments(); break;
                case MI_SelectOddSegments:
                  MovieCutterSelectOddSegments(); /*if(!DirectSegmentsCut) {ActionMenuDraw(); State = ST_ActionMenu;}*/
                  break;
                case MI_SelectEvenSegments:
                  MovieCutterSelectEvenSegments(); /*if(!DirectSegmentsCut) {ActionMenuDraw(); State = ST_ActionMenu;}*/
                  break;
                case MI_ClearAll:
...
// ----------------------------------------------------------------------------
void
ShowConfirmationDialog(char* MessageStr)
{
  TRACEENTER();
...
void
ShowErrorMessage(char* MessageStr)
{
  TRACEENTER();
...
CleanupCut(void)
{
  ...
  for(i = 0; i < NrFiles; i++)
  {
    if(FolderEntry.attr == ATTR_NORMAL)
    {
      strcpy(RecFileName, FolderEntry.name);
      strcpy(MpgFileName, FolderEntry.name);
      if(StringEndsWith(RecFileName, ".cut.bak") == 0)
      {
...
LoadINI(void)
{
  ...
  INICloseFile();
  if(IniFileState == INILOCATION_NewFile) SaveINI();
  HDD_TAP_PopDir();
...
// ----------------------------------------------------------------------------
void
AddBookmarksToSegmentList(void)
{
  int i;
...
bool
AddSegmentMarker(dword Block, bool RejectSmallSegments)
{
  int i, j;
  dword newTime;
  char StartTime[16];
...
bool
MoveSegmentMarker(dword newBlock)
{
  dword newTime;
  int NearestMarkerIndex = FindNearestSegmentMarker();
...
bool
DeleteSegmentMarker(word MarkerIndex)
{
  int i;
...
void
ExportSegmentsToBookmarks(void)
{
  int i;
...
bool
DeleteBookmark(word BookmarkIndex)
{
  bool ret = FALSE;
  int i;
...
CutFileLoad(void)
{
  ...
  if(Version > 1) TAP_Hdd_Fread(&Padding, sizeof(byte), 1, fCut); // read the second byte of Version (if not V.1)  [COMPATIBILITY LAYER]
  ...
  if(Version == 1) TAP_Hdd_Fread(&Padding, sizeof(word), 1, fCut); // read the second word of NrSegmentMarker (if V.1)  [COMPATIBILITY LAYER]
      ...
      for(i = 1; i <= NrSegmentMarker - 2; i++)
      {
        ...
        else
        {
          while((SegmentMarker[i].Timems > CurTimeStamp->Timems) && (CurTimeStamp < TimeStamps + NrTimeStamps - 1))
            CurTimeStamp++;
          if(SegmentMarker[i].Timems < CurTimeStamp->Timems) CurTimeStamp--;
  ...
  if(SegmentMarker[NrSegmentMarker - 1].Block != PlayInfo.totalBlock)
  {
    ...
    SegmentMarker[NrSegmentMarker - 1].Timems =
        (newTime) ? newTime : (dword) (1000 * (60 * PlayInfo.duration + PlayInfo.durationSec));
  ...
  if(!LinearTimeMode && (Version == 1) && (RecFileSize == SavedSize))
  {
    ...
    for(i = 0; i < NrSegmentMarker; i++)
    {
      ...
      TAP_SPrint(LogString, " %s%2u.)  BlockNr=%8u   oldTimeStamp=%s   newTimeStamp=%s",
          (labs(oldTime * 1000 - newTime) > 1000) ? "!!" : "  ", i + 1, SegmentMarker[i].Block, oldTimeStr, newTimeStr);
      WriteLogMC(PROGRAM_NAME, LogString);
    }
  }
...
CutDumpList(void)
{
  ...
  for(i = 0; i < NrSegmentMarker; i++)
  {
    ...
    TAP_SPrint(LogString, "%02d: %010d %s %03d %3s %3s", i, SegmentMarker[i].Block, TimeString, (int) SegmentMarker[i].Percent,
        SegmentMarker[i].Selected ? "yes" : "no", (i == ActiveSegment ? "*" : ""));
    WriteLogMC(PROGRAM_NAME, LogString);
  }
...
OSDSegmentListDrawList(void)
{
  ...
  const int Scrollbar_X = 150, Scrollbar_Y = 40, /*ScrollbarWidth = 10,*/
      ScrollbarHeight = 256;
...
//          else
        ...
        PosX += NrWidth;
        FMUC_PutString(rgnSegmentList, PosX, PosY, PosX + TimeWidth,
            (Start + i == 0) ? LangGetString(LS_BeginStr) : StartTime, UseColor, COLOR_None, &Calibri_12_FontDataUC, FALSE, ALIGN_RIGHT);
        PosX += TimeWidth;
        FMUC_PutString(rgnSegmentList, PosX, PosY, PosX + DashWidth, "-", UseColor, COLOR_None, &Calibri_12_FontDataUC, FALSE, ALIGN_CENTER);
        PosX += DashWidth;
        FMUC_PutString(rgnSegmentList, PosX, PosY, EndTextField_X + 10,
            (Start + i == NrSegmentMarker - 2) ? LangGetString(LS_EndStr) : EndTime, UseColor, COLOR_None, &Calibri_12_FontDataUC, FALSE, ALIGN_LEFT);
      }
    }
...
// ------------------
void
OSDInfoDrawProgressbar(bool Force)
{
  int y, i;
  dword pos = 0;
  dword x1, x2;
...
//          else
        ...
        for(i = 1; i < NrSegmentMarker - 1; i++)
        {
          ...
          if(totalBlock && SegmentMarker[i].Block <= totalBlock)
          {
            ...
            TAP_Osd_PutGd(rgnInfo, 31 + pos, 93, (
                (BookmarkMode) ? &_SegmentMarker_gray_Gd : &_SegmentMarker_Gd), TRUE);
          }
        }
        ...
        for(i = 0; i < NrBookmarks; i++)
        {
          if(totalBlock && (Bookmarks[i] <= totalBlock))
          {
            ...
            TAP_Osd_PutGd(rgnInfo, 31 + pos, 112, (
                (BookmarkMode) ? &_BookmarkMarker_Gd : &_BookmarkMarker_gray_Gd), TRUE);
          }
        }
...
void
OSDInfoDrawCurrentPosition(bool Force)
{
  dword Time;
  float Percent;
  char TimeString[24];
...
void
OSDInfoDrawPlayIcons(bool Force)
{
  ...
  if(rgnInfo)
  {
    ...
    if(Force || (TrickMode != LastTrickMode) || (TrickModeSwitch != LastTrickModeSwitch))
    {
      ...
      {
        case TRICKMODE_Normal:
          Btn_Play = &_Button_Play_Active_Gd;
          break;
        case TRICKMODE_Forward:
          Btn_Fwd = &_Button_Ffwd_Active_Gd;
          break;
        case TRICKMODE_Rewind:
          Btn_Rwd = &_Button_Rwd_Active_Gd;
          break;
        case TRICKMODE_Slow:
          Btn_Slow = &_Button_Slow_Active_Gd;
          break;
        case TRICKMODE_Pause:
          Btn_Pause = &_Button_Pause_Active_Gd;
          break;
      }
      ...
      {
        case 0x11:
          FMUC_PutString(rgnInfo, 657, 26, 697, "2x", COLOR_White, COLOR_None, &Calibri_12_FontDataUC, TRUE, ALIGN_CENTER);
          break;
        case 0x12:
          FMUC_PutString(rgnInfo, 657, 26, 697, "4x", COLOR_White, COLOR_None, &Calibri_12_FontDataUC, TRUE, ALIGN_CENTER);
          break;
        case 0x13:
          FMUC_PutString(rgnInfo, 657, 26, 697, "8x", COLOR_White, COLOR_None, &Calibri_12_FontDataUC, TRUE, ALIGN_CENTER);
          break;
        case 0x14:
          FMUC_PutString(rgnInfo, 657, 26, 697, "16x", COLOR_White, COLOR_None, &Calibri_12_FontDataUC, TRUE, ALIGN_CENTER);
          break;
        case 0x15:
          FMUC_PutString(rgnInfo, 657, 26, 697, "32x", COLOR_White, COLOR_None, &Calibri_12_FontDataUC, TRUE, ALIGN_CENTER);
          break;
        case 0x16:
          FMUC_PutString(rgnInfo, 657, 26, 697, "64x", COLOR_White, COLOR_None, &Calibri_12_FontDataUC, TRUE, ALIGN_CENTER);
          break;
        case 0x21:
          FMUC_PutString(rgnInfo, 549, 26, 589, "2x", COLOR_White, COLOR_None, &Calibri_12_FontDataUC, TRUE, ALIGN_CENTER);
          break;
        case 0x22:
          FMUC_PutString(rgnInfo, 549, 26, 589, "4x", COLOR_White, COLOR_None, &Calibri_12_FontDataUC, TRUE, ALIGN_CENTER);
          break;
        case 0x23:
          FMUC_PutString(rgnInfo, 549, 26, 589, "8x", COLOR_White, COLOR_None, &Calibri_12_FontDataUC, TRUE, ALIGN_CENTER);
          break;
        case 0x24:
          FMUC_PutString(rgnInfo, 549, 26, 589, "16x", COLOR_White, COLOR_None, &Calibri_12_FontDataUC, TRUE, ALIGN_CENTER);
          break;
        case 0x25:
          FMUC_PutString(rgnInfo, 549, 26, 589, "32x", COLOR_White, COLOR_None, &Calibri_12_FontDataUC, TRUE, ALIGN_CENTER);
          break;
        case 0x26:
          FMUC_PutString(rgnInfo, 549, 26, 589, "64x", COLOR_White, COLOR_None, &Calibri_12_FontDataUC, TRUE, ALIGN_CENTER);
          break;
        case 0x31:
          FMUC_PutString(rgnInfo, 602, 26, 642, "1/2x", COLOR_White, COLOR_None, &Calibri_12_FontDataUC, TRUE, ALIGN_CENTER);
          break;
        case 0x32:
          FMUC_PutString(rgnInfo, 602, 26, 642, "1/4x", COLOR_White, COLOR_None, &Calibri_12_FontDataUC, TRUE, ALIGN_CENTER);
          break;
        case 0x33:
          FMUC_PutString(rgnInfo, 602, 26, 642, "1/8x", COLOR_White, COLOR_None, &Calibri_12_FontDataUC, TRUE, ALIGN_CENTER);
          break;
        case 0x34:
          FMUC_PutString(rgnInfo, 602, 26, 642, "1/16x", COLOR_White, COLOR_None, &Calibri_12_FontDataUC, TRUE, ALIGN_CENTER);
          break;
      }
...
OSDInfoDrawBookmarkMode(void)
{
  ...
  if(rgnInfo)
  {
    ...
    FMUC_PutString(rgnInfo, BookmarkMode_x, 70, BookmarkMode_x + 60, ((
        BookmarkMode ? LangGetString(LS_Bookmarks) : LangGetString(LS_Segments))), (
        (BookmarkMode) ? RGB(60,255,60) : RGB(250,139,18) ), COLOR_None, &Calibri_10_FontDataUC, FALSE, ALIGN_CENTER);
    TAP_Osd_Sync();
  }
...
Playback_FFWD(void)
{
  ...
  if(TrickMode != TRICKMODE_Forward) TrickModeSpeed = 0;
...
Playback_RWD(void)
{
  ...
  if(TrickMode != TRICKMODE_Rewind) TrickModeSpeed = 0;
...
void
Playback_JumpNextBookmark(void)
{
  int i;
...
void
Playback_JumpPrevBookmark(void)
{
  TRACEENTER();
...
//    ActionMenuItem = 0;
  ...
  for(i = 0; i < MI_NrMenuItems; i++)
  {
    ...
    switch(i)
    {
      ...
      {
        DisplayStr =
            (NrSegmentMarker == 4) ? LangGetString(LS_SelectPadding) : LangGetString(LS_SelectOddSegments);
        if(DirectSegmentsCut)
          DisplayStr =
              (NrSegmentMarker == 4) ? LangGetString(LS_RemovePadding) : LangGetString(LS_DeleteOddSegments);
        if(NrSegmentMarker <= 2) DisplayColor = C4;
        break;
      }
      case MI_SelectEvenSegments:
      {
        DisplayStr =
            (NrSegmentMarker == 4) ? LangGetString(LS_SelectMiddle) : LangGetString(LS_SelectEvenSegments);
        if(DirectSegmentsCut) DisplayStr = LangGetString(LS_DeleteEvenSegments);
        if(NrSegmentMarker <= 2) DisplayColor = C4;
        break;
      }
      case MI_ClearAll:
    ...
    if(DisplayStr && (i < MI_NrMenuItems))
      FMUC_PutString(rgnActionMenu, 18, 4 + 28 * i, 300, DisplayStr, DisplayColor, COLOR_None, &Calibri_14_FontDataUC, TRUE,
          (i == 0) ? ALIGN_CENTER : ALIGN_LEFT);
  }
...
void
MovieCutterDeleteSegments(void)
{
  TRACEENTER();
...
void
MovieCutterSelectOddSegments(void)
{
...
void
MovieCutterSelectEvenSegments(void)
{
  TRACEENTER();
...
void
MovieCutterProcess(bool KeepCut)
{
//  int                   NrSelectedSegments;
...
//  OSDRedrawEverything();
  ...
  if(State != ST_UnacceptedFile) State = ST_WaitForPlayback;
  NoPlaybackCheck = FALSE;
...
byte
PlaybackRepeatMode(bool ChangeMode, byte RepeatMode, dword RepeatStartBlock, dword RepeatEndBlock)
{
...
bool
PlaybackRepeatSet(bool EnableRepeatAll)
{
...
// ----------------------------------------------------------------------------
dword
NavGetBlockTimeStamp(dword PlaybackBlockNr)
{
  TRACEENTER();
  ...
  if(LastTimeStamp->BlockNr < PlaybackBlockNr)
  {
    ...
    while((LastTimeStamp->BlockNr < PlaybackBlockNr) && (LastTimeStamp < TimeStamps + NrTimeStamps - 1))
      LastTimeStamp++;
    if(LastTimeStamp->BlockNr > PlaybackBlockNr) LastTimeStamp--;
  }
...
bool
PatchOldNavFile(char *SourceFileName, bool isHD)
{
...
bool
PatchOldNavFileHD(char *SourceFileName)
{
  FILE *fSourceNav;
  TYPE_File *fNewNav;
  char FileName[MAX_FILE_NAME_SIZE + 1];
