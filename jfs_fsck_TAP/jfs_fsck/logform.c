...
{				/* volume label for external log, none if NULL */
	...
	if (log_sup == NULL) {
		TAP_PrintNet("logform: calloc failed!");
		return -1;
	}
...
#define MAX_LOG_SIZE 128*1048576	/* 128 MB */
		if (log_len_in_bytes > MAX_LOG_SIZE) {
			TAP_PrintNet("External journal size is excessive.\n"
		...
		if (uuid_is_null(uuid))
			...
			if (!uuid_compare(log_sup->uuid, uuid)) {
				TAP_PrintNet("Invalid log device");
				return -1;
			}
		}
	}
	...
	for (k = 1; k < npages - 4; k += 4) {
		...
		if (isatty(STDOUT_FILENO)) {
			...
			case (100):
				TAP_PrintNet("%s", Working[0]);
				fflush(stdout);
				break;
			case (200):
				TAP_PrintNet("%s", Working[1]);
				fflush(stdout);
				break;
			case (300):
				TAP_PrintNet("%s", Working[2]);
				fflush(stdout);
				break;
			case (400):
				TAP_PrintNet("%s", Working[3]);
				fflush(stdout);
				Working_counter = 0;
				break;
			default:
