		 ...
		 uint32_t sb_flag)
{
	...
	if (which_table == AGGREGATE_I) {
		...
		if (inum >= NUM_INODE_PER_EXTENT) {
			TAP_PrintNet("Internal error: %s(%d): Aggregate inode out of range (%d)", __FILE__, __LINE__, inum);
		...
		TAP_PrintNet("Internal error: %s(%d): Bad map inode number (%d)", __FILE__, __LINE__, which_table);
		 ...
		 int32_t fs_block_size)
{
		...
		if (cmp == 0) {
			...
			if (page->header.flag & BT_LEAF) {
				*offset = (addressXAD(&(page->xad[index])) +
				...
				if (rc) {
					TAP_PrintNet("Internal error: %s(%d): Error reading btree node", __FILE__, __LINE__);
					return rc;
				}
	...
	/* Not found! */
	TAP_PrintNet("Internal error: %s(%d): Block %lld not found!", __FILE__, __LINE__, (long long) lbno);
	return EINVAL;
	...
	case GET:
		...
		*offset = (disk_extent_offset + lbno) * fs_block_size;
		break;
	case PUT:
		TAP_PrintNet("Internal error: %s(%d): does not handle PUT", __FILE__, __LINE__);
		return EPERM;
		break;
	default:
		return EINVAL;
