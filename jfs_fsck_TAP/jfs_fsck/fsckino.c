 ...
 *      failure: something else
 */
int display_paths(uint32_t inoidx, struct fsck_inode_record *ino_recptr, struct fsck_ino_msg_info *msg_info_ptr)
{
	int dips_rc = FSCK_OK;
	int intermed_rc;
...
int inode_is_in_use(struct dinode *inode_ptr, uint32_t inode_num)
{
	...
	ixpxd_unequal = memcmp((void *) &(inode_ptr->di_ixpxd), (void *) &(agg_recptr->ino_ixpxd), sizeof (pxd_t));
 ...
 *      failure: something else
 */
int validate_record_fileset_inode(uint32_t inonum, uint32_t inoidx, struct dinode *inoptr, struct fsck_ino_msg_info *ino_msg_info_ptr)
{
	int                   vrfi_rc = FSCK_OK;
	int                   intermed_rc = FSCK_OK;
	struct                fsck_inode_record *inorecptr;
	...
	//TAP_PrintNet("validate_record_fileset_inode(%d, %d, %p, %p)", inonum, inoidx, inoptr, ino_msg_info_ptr);
	...
	if (!(inode_type_recognized(inoptr)))
	{
		 ...
		 */
		agg_recptr->corrections_needed = 1;
		fsck_send_msg(fsck_BADINOTYP, fsck_ref_msg(ino_msg_info_ptr->msg_inopfx), ino_msg_info_ptr->msg_inonum);
	 ...
	 */
	memset((void *) (&(agg_recptr->this_inode)), '\0', sizeof (agg_recptr->this_inode));
	memcpy((void *) &(agg_recptr->this_inode.eyecatcher), (void *) "thisinod", 8);
	...
	if (inorecptr->inode_type == directory_inode)
	{
		 ...
		 */
		vrfi_rc = validate_dir_data(inoptr, inoidx, inorecptr, ino_msg_info_ptr);
	}
	else if	(ISREG(inoptr->di_mode) || ISLNK(inoptr->di_mode))
	{
		/*
		 ...
		 */
		vrfi_rc = validate_data(inoptr, inoidx, inorecptr, ino_msg_info_ptr);
	}
	if (vrfi_rc != FSCK_OK)
		goto out;
	...
	if (!inorecptr->selected_to_rls)
	{
		...
		if (inoptr->di_nblocks != agg_recptr->this_inode.all_blks)
		{
		  ...
		  //display_paths(inoidx, inorecptr, ino_msg_info_ptr);
			TAP_PrintNet("wrong number of blocks: expected=%lld, received=%lld", agg_recptr->this_inode.all_blks, inoptr->di_nblocks);

			fsck_send_msg(fsck_BADKEYS, fsck_ref_msg(ino_msg_info_ptr->msg_inotyp), fsck_ref_msg(ino_msg_info_ptr->msg_inopfx), ino_msg_info_ptr->msg_inonum, 9);
			...
			if (agg_recptr->processing_readwrite)
			{
			  ...
			  if ((icheck_return > 0) && (icheck_return & 0x04))
			  {
			    ...
			    fsck_send_msg(mc_FIXEDNBLOCKSVALUE, ino_msg_info_ptr->msg_inonum);
			    //mc_NrFixedFiles++;
			  }
			  else
			    fsck_send_msg(mc_ERRORFIXINGNBLOCKS, ino_msg_info_ptr->msg_inonum, icheck_return);
			}
			else
			{
		...
		else
		{
			...
			if (!ISDIR(inoptr->di_mode) && ((inoptr->di_size < min_size) || (!(inoptr->di_mode & ISPARSE) && (inoptr->di_size > max_size))))
			{
				 ...
				 * - tree must be bad.
				 */
				fsck_send_msg(fsck_BADKEYS, fsck_ref_msg(ino_msg_info_ptr->msg_inotyp), fsck_ref_msg(ino_msg_info_ptr->msg_inopfx), ino_msg_info_ptr->msg_inonum, 10);
	...
	if (!(inorecptr->ignore_alloc_blks))
	{
		...
		if (inorecptr->selected_to_rls)
		{
			fsck_send_msg(fsck_BADKEYS, fsck_ref_msg(ino_msg_info_ptr->msg_inotyp), fsck_ref_msg(ino_msg_info_ptr->msg_inopfx), ino_msg_info_ptr->msg_inonum, 39);
		...
		else
		{
			...
			if ((!(agg_recptr->this_inode.ea_inline) || (inorecptr->clr_ea_fld)) && (!(agg_recptr->this_inode.acl_inline) || (inorecptr->clr_acl_fld)))
			{
				/*
			 ...
			 */
			if ((dinode_sect4_avail) && ((inoptr->di_mode & INLINEEA) != INLINEEA))
			{
				inorecptr->inlineea_on = 1;
				agg_recptr->corrections_needed = 1;
			}
			else if ((!dinode_sect4_avail) && ((inoptr->di_mode & INLINEEA) == INLINEEA))
			{
				/*
	...
	else
	{
		...
		if (bad_size)
		{
			...
			if (inorecptr->inode_type == directory_inode)
			{
				 ...
				 * extent or a B+ Tree
				 */
				process_valid_dir_data(inoptr, inoidx, inorecptr, ino_msg_info_ptr, FSCK_UNRECORD);
			}
			else
			{
				/*
				 ...
				 * extent or a B+ Tree
				 */
				process_valid_data(inoptr, inoidx, inorecptr, ino_msg_info_ptr, FSCK_UNRECORD);
			}
		}
	}
	...
	if ((vrfi_rc == FSCK_OK) && (!inorecptr->selected_to_rls))
	{
		...
		{
			/* a directory */
			agg_recptr->blocks_for_dirs = agg_recptr->blocks_for_dirs + agg_recptr->this_inode.all_blks - agg_recptr->this_inode.ea_blks - agg_recptr->this_inode.acl_blks;
		}
		else
		{
			/* a file */
			agg_recptr->blocks_for_files = agg_recptr->blocks_for_files + agg_recptr->this_inode.all_blks - agg_recptr->this_inode.ea_blks - agg_recptr->this_inode.acl_blks;
		}
	}
