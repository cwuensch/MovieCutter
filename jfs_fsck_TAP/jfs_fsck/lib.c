...
int find_inode(unsigned inum, unsigned which_table, int64_t * address)
{
	...
	if (rc) {
		fputs("find_inode: Error reading iag", stderr);
		return 1;
	}
...
int find_iag(unsigned iagnum, unsigned which_table, int64_t * address)
{
	...
	if (which_table != FILESYSTEM_I &&
		...
		TAP_PrintNet("find_iag: Invalid fileset, %d", which_table);
		return 1;
	}
	...
	if (rc) {
		fputs("find_inode: Error reading fileset inode", stderr);
		return 1;
	}
			...
			if (page->header.flag & BT_LEAF) {
				*address = (addressXAD(&(page->xad[index]))
				...
				rc = xRead(addressXAD(&(page->xad[index])) << l2bsize,
					   PSIZE, buffer);
				if (rc) {
					fputs("find_iag: Error reading btree node", stderr);
					return 1;
				}
	...
	/* Not found! */
	TAP_PrintNet("find_iag:  IAG %d not found!", iagnum);
	return 1;
}
