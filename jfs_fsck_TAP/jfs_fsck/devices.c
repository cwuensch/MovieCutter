...
int ujfs_get_dev_size(FILE *device, int64_t *size)
{
...
#if defined(__DragonFly__)
	{
		...
		DBG_TRACE(("ujfs_get_device_size: slice = %d", slice));
		...
		DBG_TRACE(("ujfs_get_device_size: size in bytes = %ld", *size));
		       ...
		       int32_t mode)
{
	...
	if (disk_count == 0) {
		TAP_PrintNet("ujfs_rw_diskblocks: disk_count is 0");
	...
	case PUT:
		...
		break;
	default:
		DBG_ERROR(("Internal error: %s(%d): bad mode: %d", __FILE__,
			   __LINE__, mode))
	...
	if (Bytes_Transferred != disk_count) {
		...
		{
			TAP_PrintNet("ujfs_rw_diskblocks");
		}
		else
		{
			TAP_PrintNet("ujfs_rw_diskblocks: %s %zd of %d bytes at offset %lld", (mode == GET) ? "read" : "wrote", Bytes_Transferred, disk_count, disk_offset);
		}
